import { DeleteOutlined, EditOutlined, ExclamationCircleFilled, FileAddOutlined, SaveOutlined } from '@ant-design/icons'
import { Button, Image, Row, Table, Modal, Col, message, Divider, Tooltip, PageHeader } from 'antd'
...
function Work_user_page() {
    ...
    const columns = [
        ...
        {
            title: "Зураг",
            render: (_, row) => <><Image src={row?.profile_img} width="50px" height="50px" /></>,
            key: "phone"
        },
        {
            title: "И-мэйл",
        ...
        {
            title: "Үйлдэл",
            render: (_, row) => <><Button icon={<EditOutlined />} onClick={() => showModal(row.id)} /> <Button icon={<DeleteOutlined onClick={() => showDeleteConfirm(row.id)} />} /></>,
        }
    ]
    ...
    return (
        <>
            <PageHeader title="Ажилтан" extra={<Button onClick={onCreate} icon={<FileAddOutlined />}>Нэмэх</Button>} />
            <Table columns={columns} dataSource={userlist} />
            <Modal title={`${oneuserdata?.first_name == "" ? "Хэрэглэгч нэмэх" : oneuserdata?.first_name} ${oneuserdata?.last_name}  Засварлах`} open={isModalOpen} onCancel={() => setIsModalOpen(false)} footer={false}>
                <Formik onSubmit={onSubmit} initialValues={oneuserdata} validationSchema={validationSchema} enableReinitialize>
                    <Form layout='vertical'>
                        ...
                        <Button style={{ width: "100%" }} htmlType="submit" icon={<SaveOutlined />}>Хадаглаж</Button>
                    </Form>
                </Formik>
            </Modal>
            <ToastContainer />
